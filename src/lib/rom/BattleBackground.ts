...
export default class BattleBackground {
  ...
  * 16 Effects
  */
  private bbgData = new Uint8Array(STRUCT_SIZE);

  constructor (private rom: ROM, i = 0) {
    this.read(i)
  }
  /**
  ...
  */
  ...
  read (index: number) {
    const main = this.rom.readBlock(0xDCA1 + index * STRUCT_SIZE)
    for (let i = 0; i < STRUCT_SIZE; ++i) {
      this.bbgData[i] = main.readInt8()
    }
  }
}
